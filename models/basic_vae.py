...
class Adapter(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.downsample = nn.Conv2d(in_channels, in_channels, kernel_size=4, stride=4, padding=0)
    
    def forward(self, x):
        identity = x
        out = self.downsample(x)
        return out + F.interpolate(identity, size=out.shape[2:], mode='bilinear', align_corners=True)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.input_adapter = Adapter(self.ch)
